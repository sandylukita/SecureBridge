...
This guide details how to deploy SecureBridge to **Microsoft Azure** as an online showcase lab. By combining a lightweight Azure Virtual Machine with **Google Gemini Flash API (Free Tier)**, you get an ultra-fast, publicly accessible OT SOC Security Dashboard at minimal cost.
...
[Client / Interviewer] ──(HTTPS/HTTP Port 8501)──> [Azure Network Security Group]
                                                ...
                                                [Azure Ubuntu VM (B2s)]
                                                 ...
                                                 ├── Modbus Traffic Simulator
                                                 ├── 19-Feature Isolation Forest
                                                 └── Gemini Flash API (<1s AI)
```

### 💰 Cost Comparison Profile

| Metric | Local Ollama on Azure | Gemini API on Azure (Recommended) |
|---|---|---|
...
| **LLM Inference Cost** | $0 | **$0 (Google Gemini Free Tier)** |
| **Inference Speed** | 5 – 10 seconds | **Sub-second (< 1 second)** |
...
3. Configure the **Basics** tab:
   ...
   - **Region**: Select closest region e.g. `(Asia Pacific) Southeast Asia` or `(US) East US`.
...
```bash
ssh azureuser@securebridge-demo.eastus.cloudapp.azure.com
...
Create your `.env` file and insert your Gemini API Key:
...
### 4. Verify & Update Configuration (`config/lab.yaml`)

Ensure `config/lab.yaml` uses `mode: auto` (or `mode: gemini`):
...
You should see output indicating Streamlit is running on `http://0.0.0.0:8501`.
...
You will see the **SecureBridge SOC Security Dashboard** live with:
- 3 Simulated Modbus PLCs actively sending OT telemetry
- 19-Feature Isolation Forest scoring live anomalies
- Instant sub-second AI Threat Analysis powered by **Google Gemini API**
- Persistent PDF Compliance Report generation directly in the sidebar!
...
> *"For remote showcases and lab demonstrations, we host SecureBridge on a lightweight Azure VM integrated with Google Gemini API for sub-second threat analysis. When deploying at live critical infrastructure sites (e.g. Oil & Gas refineries), SecureBridge switches to 100% Air-Gapped mode running on-premise local Ollama LLMs so that zero network telemetry ever leaves the plant boundary."*
